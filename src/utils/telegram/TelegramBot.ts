...
function getTxHashURLfromTenderly(txHash: string) {
...
function getDollarAddOnWithoutBrakets(amountStr: any) {
  let amount;
  if (typeof amountStr === "string") {
    amount = parseFloat(amountStr.replace(/,/g, ""));
  } else {
...
export async function buildTokenExchangeMessage(formattedEventData: any, source: string) {
  ...
  if (!theOtherCoinName) return;
  if (!theOtherCoinAddress) return;
...
export async function buildRemoveLiquidityMessage(formattedEventData: any, source: string) {
  ...
  let removedWhat = `${amountUSDC ? `${formatForPrint(amountUSDC)}${hyperlink(getTokenURL(theOtherCoinAddress), theOtherCoinName)} | ` : ""}${
    amountWBTC ? `${formatForPrint(amountWBTC)}${hyperlink(getTokenURL(ADDRESS_WBTC), "WBTC")} | ` : ""
  }${amountWETH ? `${formatForPrint(amountWETH)}${hyperlink(ethUrl, ethName)}` : ""}`.trim();
...
export async function buildAddLiquidityMessage(formattedEventData: any, source: string) {
  ...
  let addedWhat = `${addedUSDC ? `${formatForPrint(addedUSDC)}${hyperlink(getTokenURL(theOtherCoinAddress), theOtherCoinName)} | ` : ""}${
    addedWBTC ? `${formatForPrint(addedWBTC)}${hyperlink(getTokenURL(ADDRESS_WBTC), "WBTC")} | ` : ""
  }${addedWETH ? `${formatForPrint(addedWETH)}${hyperlink(ethUrl, ethName)}` : ""}`.trim();
