...
export async function subscribeToEvents(CONTRACT: any, contractName: string, eventEmitter: EventEmitter) {
  try {
    ...
    subscription
      ...
      .on("data", async (eventData: any) => {
        // attach source to the eventData
        eventData.source = contractName;
...
export async function getTxData(txHash: string) {
  return await WEB3_HTTP_PROVIDER.eth.getTransaction(txHash);
}
